...
(defvar *resources-path*
  (asdf:system-relative-pathname :cl-tiled-demo #P"Resources/"))
...
(defun init ()
  (setf *map* (tiled:load-map "level1.tmx"))
...
(defvar *tileset-images* (make-hash-table :test #'equal))

(defmethod render-layer ((layer tiled:tile-layer))
  (dolist (cell (tiled:layer-cells layer))
    (let* ((dx (tiled:cell-x cell))
           (dy (tiled:cell-y cell))
           (tile (tiled:cell-tile cell))
           (sx (tiled:tile-pixel-x tile))
           (sy (tiled:tile-pixel-y tile))
           (w (tiled:tile-width tile))
           (h (tiled:tile-width tile))
           (tileset (tiled:tile-tileset tile))
           (image-source (namestring
                          (tiled:image-source (tiled:tileset-image tileset))))
           (image (ensure-gethash image-source *tileset-images*
                                  (al:load-bitmap image-source))))
      (al:draw-bitmap-region image sx sy w h dx dy 0))))
...
(cffi:defcallback %main :int ((argc :int) (argv :pointer))
  ...
  (handler-bind
    ...
    (let ((display (al:create-display +window-width+ +window-height+))
      ...
      (al:set-window-title display "cl-tiled demo")
